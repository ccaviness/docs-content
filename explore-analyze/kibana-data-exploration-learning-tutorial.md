...
1. Select {icon}`dashboardApp` **Save visualization to dashboard** (or {icon}`save` **Save visualization** in earlier versions) above the chart. You can also select {icon}`pencil` **Edit visualization** to open the Lens editor inline and customize the chart before saving it.
...
:   When a classic search or an {{esql}} aggregation produces a chart in Discover, select {icon}`dashboardApp` **Save visualization to dashboard** (or {icon}`save` **Save visualization** in earlier versions) above the chart, then choose **Add to dashboard** to send it to an existing or new dashboard. You can also save the entire Discover session (query, filters, and selected fields) and add it to a dashboard as a table panel from the Discover table toolbar or the dashboard library.
