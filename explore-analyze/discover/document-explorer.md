...
To save the current view of the documents table as a panel on a new or existing dashboard, select {icon}`dashboardApp` **Save table to dashboard** in the table toolbar. For the full workflow and requirements, refer to [Save the table directly from Discover](save-open-search.md#save-table-to-dashboard).
